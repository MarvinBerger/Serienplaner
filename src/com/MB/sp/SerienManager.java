...
public class SerienManager extends SQLiteOpenHelper {
	...
	public Serie[] getSeries() {
		...
		do {
			...
			for (int i = 0; i < d.length; i++)
				d[i] = new Date(
						Long.parseLong(c.getString(2).split(";")[i]) * 1000);

			out[index++] = new Serie(c.getString(0), c.getString(4),
					c.getString(2), d, c.getInt(3), c.getInt(1));
		} while (c.moveToNext());
		//TEST TEIL
		out = new Serie[1];
	...
	public Serie getSerie(int id) {
		Cursor c = this.getReadableDatabase().rawQuery(
				SERIE_PER_ID.replace("#ersetzen#", "" + id), null);
		c.moveToFirst();
		Date[] d = new Date[c.getString(2).split(";").length];
		for (int i = 0; i < d.length; i++)
			d[i] = new Date(Long.parseLong(c.getString(2).split(";")[i]) * 1000);
		return new Serie(c.getString(0), c.getString(4), c.getString(2), d,
				c.getInt(3), c.getInt(1));
